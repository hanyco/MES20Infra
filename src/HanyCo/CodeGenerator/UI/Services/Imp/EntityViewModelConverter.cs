...
internal sealed class EntityViewModelConverter : IEntityViewModelConverter
{
    ...
    public DtoViewModel FillByDbEntity(in Dto dto, in IEnumerable<Property>? properties)
    {
        ...
        if (!dto.DbObjectId.IsNullOrEmpty())
        {
    ...
    {
        Check.ArgumentNotNull(dto);
        Check.ArgumentNotNull(properties);
    ...
    public Property? ToDbEntity(PropertyViewModel? viewModel)
    {
        ...
        //.ForMember(x => x.Dto = viewModel.Dto?.Id is null ? null : new() { Id = viewModel.Dto.Id.Value });
        return result;
    }
    ...
    public CqrsSegregate? ToDbEntity(CqrsQueryViewModel? model)
        => this.ToDbEntityInner(model, CqrsSegregateType.Query);
    ...
    public CqrsSegregate? ToDbEntity(CqrsCommandViewModel? model)
        => this.ToDbEntityInner(model, CqrsSegregateType.Command);
    ...
    public IEnumerable<FunctionalityViewModel?> ToViewModel(IEnumerable<Functionality?> entities) => throw new NotImplementedException();

    public FunctionalityViewModel? ToViewModel(Functionality? entity) => throw new NotImplementedException();

    private CqrsSegregate? ToDbEntityInner(CqrsViewModelBase? model, CqrsSegregateType segregateType)
        => model is null
                ? null
