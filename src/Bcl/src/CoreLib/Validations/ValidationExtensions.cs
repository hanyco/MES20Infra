...
public static class ValidationExtensions
{
    public static ValidationResultSet<TValue> Any<TValue>(this ValidationResultSet<TValue> vrs, Func<Exception> onError = null)
            ...
            vrs.InnerAddRule(x => x, (_) => vrs.Value.Any(), onError, () => new NoItemValidationException(vrs._valueName));
    ...
    /// <returns>The value if it is not null.</returns>
    [return: NotNull]
    public static TValue NotNull<TValue>([NotNull] this TValue value, Func<Exception> onError) =>
    ...
    public static ValidationResultSet<TValue> NotNull<TValue>(this ValidationResultSet<TValue> vrs, Expression<Func<TValue, object>> propertyExpression, Func<string> onErrorMessage) =>
        vrs.InnerAddRule(propertyExpression, x => x is not null, null, x => new NullValueValidationException(onErrorMessage?.Invoke() ?? x, null));
    ...
    private static Result<TValue> InnerBuild<TValue>(in CheckBehavior behavior, in TValue value, in IEnumerable<(Func<TValue, bool> IsValid, Func<Exception> OnError)> rules)
    {
        ...
        foreach (var (isValid, onError) in rules)
        {
            ...
            if (!isValid(value))
            {
                ...
                var errorResult = Result.Fail<TValue>(exception, value);
                switch (behavior)
                {
                    // Combine all errors
