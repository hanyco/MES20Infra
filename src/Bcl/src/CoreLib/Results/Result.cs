    ...
    , ICombinable<ResultBase, Result>
{
    ...
    public static Result Fail(in string? message) => Fail(message, ImmutableArray<Exception>.Empty);

    public static Result Fail(in string? message, in IEnumerable<Exception>? errors) => new(false, message, errors);

    public static Result Fail<TException>() where TException : Exception, new() => Fail(new TException());

    public static Result Fail(in string? message, in Exception error) => Fail(message, [error]);

    public static Result Fail(in Exception error) => Fail((string?)null, error);
    ...
    public static Result<TValue?> Fail<TValue>(in string message) => Fail<TValue>(default, message, ImmutableArray<Exception>.Empty);

    public static Result<TValue?> Fail<TValue>(in string message, in TValue? value) => Fail<TValue>(value, message, ImmutableArray<Exception>.Empty);

    public static Result<TValue?> Fail<TValue>(in TValue? value = default) => Fail<TValue?>(value, null, ImmutableArray<Exception>.Empty);
