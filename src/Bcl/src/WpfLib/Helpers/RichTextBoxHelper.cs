...
public static class RichTextBoxHelper
{
    public static FlowDocument FormatCSharpCode(string code)
    {
        ...
        var paragraphs = TextToParagraphs(code, lineProcess, wordProcess);
        ...
        (bool Found, IEnumerable<Inline>? Inline)? lineProcess((string CurrentLine, string? PrevLine) line)
        {
            ...
            return line.CurrentLine.Trim().StartsWith("///")
                ? ((bool Found, IEnumerable<Inline>? Inline)?)(true, EnumerableHelper.AsEnumerable(new Italic(new Run(line.CurrentLine)) { Foreground = Brushes.DarkGreen }))
                : line.CurrentLine.Trim().StartsWith("//")
