...
public static class ControlHelper
{
    ...
    public static void EnsureVisible(this ListViewItem item) => item.ArgumentNotNull(nameof(item)).Parent.CastAs<ListView>()?.ScrollIntoView(item);
    ...
    public static void ExpandAll(this TreeViewItem item, bool isExpanded = true)
    {
        ...
        } while ((parent = parent?.Parent.CastAs<TreeViewItem>()) is not null);
                ...
                return !loopThrouth && !Equals(c, visual)
                    ...
                    : c is not TabItem tabItem
                        ...
                        : tabItem.Content is Visual
                                            ? (IEnumerable<Visual>)(new[] { tabItem.Content.CastAs<Visual>()!, tabItem.Header.CastAs<Visual>()! })
                                            : Enumerable.Range(0, VisualTreeHelper.GetChildrenCount(c)).Select(i => (Visual)VisualTreeHelper.GetChild(c, i));
            },
            result.Add,
            null);
    ...
    public static T? GetSelectedValue<T>(this TreeView treeView) where T : class
        => treeView.ArgumentNotNull(nameof(treeView)).SelectedItem.CastAs<TreeViewItem>()?.DataContext.CastAs<T>();

    [Obsolete("Use listView.GetSelection, instead.")]
    public static T? GetSelection<T>(this SelectionChangedEventArgs e)
        => e.ArgumentNotNull(nameof(e)).AddedItems.Cast<object?>().FirstOrDefault().CastTo<T?>();

    public static T? GetSelection<T>([DisallowNull] this ListView listView, SelectionChangedEventArgs e)
        => e?.AddedItems.Any() is true ? e.AddedItems[0].CastTo<T?>() : GetSelections<T>(listView).FirstOrDefault();

    public static IEnumerable<T?> GetSelections<T>([DisallowNull] this ListView listView)
        => listView.ArgumentNotNull().SelectedItems.Cast<object?>().Select(x => x.CastTo<T?>());
    ...
    public static IEnumerable<dynamic>? RetieveCheckedItems(this MultiSelector dg)
        => dg?.Items.Cast<dynamic>().Where(item => item.IsChecked == true).Cast<object>().Select(item => item.CastAs<dynamic>()).Compact();

    public static IEnumerable<TItem>? RetieveCheckedItems<TItem>(this MultiSelector dg)
        => dg?.Items.Cast<dynamic>().Where(item => item.IsChecked == true).Cast<object>().Select(item => item.CastTo<TItem>());
