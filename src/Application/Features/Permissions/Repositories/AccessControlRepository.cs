...
internal sealed class AccessControlRepository(IdentityDbContext context) : IAccessControlRepository
{
    ...
    public async Task<AccessPermission?> GetAccessPermissionWithParentAsync(string userId, long entityId)
    {
        var query = from p in _context.AccessPermissions
                    where p.UserId == userId &&
                          (p.EntityId == entityId ||
                           p.EntityId == _context.AccessPermissions.FirstOrDefault(x => x.EntityId == entityId).ParentId)
