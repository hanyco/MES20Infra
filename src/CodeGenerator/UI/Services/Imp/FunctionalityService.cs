    ...
    , IBusinesService, IAsyncValidator<FunctionalityViewModel>, IAsyncTransactionSaveService, ILoggerContainer
{
    ...
    public async Task<Result<FunctionalityViewModel?>> GenerateAsync(FunctionalityViewModel viewModel, CancellationToken token = default)
    {
        ...
        string getTitle(string description)
            => $"{data.Result.Name} - ${description}";

        static async Task<CreationData> initialize(FunctionalityViewModel viewModel, string connectionString)
        {
            CreationData data = new(viewModel);
            var db = await Database.GetDatabaseAsync(connectionString);
            data.DbTable = db
                .NotNull().Tables[data.Result.RootDto!.DbObject.Name!]
                .NotNull(() => new Library.Exceptions.ObjectNotFoundException($"Table name `{data.Result.RootDto!.DbObject}` not found."));
            return data;
        }
        ...
        static Result<FunctionalityViewModel> validateArgs(in FunctionalityViewModel args)
        {
            ...
            var result = Check.MustBeNotNull(args, args.RootDto);
            if (result.IsSucceed)
                result += Check.MustBeNotNull(args, args.RootDto.DbObject);
    ...
    private async Task CreateGetAllQuery(CreationData data)
    {
        ...
        async Task createGetAllQueryViewModel(CreationData data)
        {
            ...
            data.Result.GetAllQueryViewModel = query.Fluent()
    ...
    private struct CreationData
    {
        ...
        public CreationData(FunctionalityViewModel result)
            => this.Result = result;

        public Table DbTable { get; set; } = default!;
