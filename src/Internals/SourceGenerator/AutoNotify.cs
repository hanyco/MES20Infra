...
");
            ...
            string chooseName(string myFieldName, TypedConstant myOverridenNameOpt)
            {
                if (!myOverridenNameOpt.IsNull)
                {
                    return myOverridenNameOpt.Value.ToString();
                }

                myFieldName = myFieldName.TrimStart('_');
                if (myFieldName.Length == 0)
                    return string.Empty;

                if (myFieldName.Length == 1)
                    return myFieldName.ToUpper();

                return myFieldName.Substring(0, 1).ToUpper() + myFieldName.Substring(1);
            }
